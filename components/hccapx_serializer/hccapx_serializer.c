 ...
 * @brief Implementation of the HCCAPX serializer module.
 * 
 * This module provides functions for serializing Wi-Fi handshake data into the HCCAPX format.
 * It includes functions for initializing the serializer, getting the serialized data, and adding new handshake frames.
 * 
...
/**
 * @brief Stores number of message from which the EAPoL packet was saved.  
 */
 ...
 * @return false if some value is different from zero
 */
...
static unsigned save_eapol(eapol_packet_t *eapol_packet, eapol_key_packet_t *eapol_key_packet){
    ...
    // Clear key MIC from EAPoL packet so hashcat can calculate MIC without preprocessing.
    // This is not documented in HCCAPX reference.
    // But it's based on 802.11i-2004 [8.5.2/h] and by analyzing the behavior of cap2hccapx tool
...
/**
 * @detail This component is a state machine, so this function can be used without knowing the current state from outside.
...
void hccapx_serializer_add_frame(data_frame_t *frame){
    ...
    // Determine the direction of the frame by comparing BSSID (addr3) with the source address (addr2)
    if(memcmp(frame->mac_header.addr2, frame->mac_header.addr3, 6) == 0){
        ...
        ESP_LOGE(TAG, "Unknown frame format. BSSID is neither the source nor the destination.");
    }
}